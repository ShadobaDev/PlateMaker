 ...
 *
 * Produced by \c ProcessingPipeline::layoutPagesFromHeaders().  The dimensions are the *render's* — they come
 * from the same page-domain code \c render() uses, not from re-derived arithmetic — so a consumer can
...
struct PagePreviewGeometry {
    ...
    InputStatus status = InputStatus::Appended; //!< Exactly what \c render() would report for this page.

    /**
     * \brief False when this page contributes nothing to the strip.
     *
     * Missing, unreadable, or failing the same checks that make \c render() skip a page — in which case
 ...
 *
 * Holds no state — every entry point is \c static; call \c ProcessingPipeline::render(...) without
...
public:
    /**
     * \brief Builds the virtual strip described by \p request, slices it, and saves every slice.
     *
     * \param request   What to render, how, and where to put it — see \c RenderRequest. Everything
     *                  optional about a render lives there, so this signature stays three arguments
     *                  however many steps the pipeline grows.
     * \param cancel    Polled once per input and once per slice; a cancelled run returns the partial
     *                  result it had reached, with \c ProcessingOutcome::cancelled set. Slices
     *                  already written stay on disk.
     * \param callbacks Optional progress/event sinks (see \c ProcessingCallbacks); any field may be
     *                  null. Invoked **synchronously on the calling thread**, so a consumer running
     *                  this on a worker owns any marshalling, and a slow callback slows the render.
     * \return A \c ProcessingOutcome with per-slice records, skipped pages and flags.
     */
    [[nodiscard]] static ProcessingOutcome render(
        const RenderRequest&                     request,
        const Infrastructure::CancellationToken& cancel,
        const ProcessingCallbacks&               callbacks = {});

    // -----------------------------------------------------------------------
    // The page domain on its own — the same code, stopped before the strip
    // -----------------------------------------------------------------------
    //
    ...
    // viewport, not the chapter. They live here, beside render(), because they must go through the same
    // page-domain code: a viewer that re-derived page geometry would drift from the render silently.
    //
    // The two are named for how they differ, because that is the only thing worth knowing when
    // choosing between them: layoutPagesFromHeaders() reads **headers** for **every** page,
    // decodePageToRgba() **decodes** **one**. The first is roughly a millisecond a page; the second
    // costs a full decode and scale. Neither is a "preview" in the sense of being approximate — the
    // geometry is the render's own.

    /**
     * \brief Every input's place on the strip, computed from file headers alone — no pixels decoded.
     *
     ...
     * dimensions, exactly as \c render() skips it. A consumer must skip those when stacking.
     *
     ...
     */
    [[nodiscard]] static std::vector<PagePreviewGeometry> layoutPagesFromHeaders(
    ...
    /**
     * \brief Decodes one page's **ungraded** pixels, at strip scale, into a caller-owned RGBA buffer.
     *
     * Runs the page exactly as \c render() would — EXIF-upright, profile-matched, margin-cropped,
     * scaled — and stops there. The result is always **8-bit sRGB RGBA8888**, four interleaved bytes
     * per pixel with no row padding, whatever the source's depth or band count was; that narrowing is
     * for the caller's convenience, and the committed render still keeps the source's format.
     *
     * \par Why ungraded
     ...
     * crosses the library boundary. Size it from this page's \c layoutPagesFromHeaders() entry: exactly
     ...
     *
     * \param input            The input file to decode (its \c uid decides colour exclusion).
     ...
     * \param width            Expected width  (from \c layoutPagesFromHeaders()).
     * \param height           Expected height (from \c layoutPagesFromHeaders()).
     *
